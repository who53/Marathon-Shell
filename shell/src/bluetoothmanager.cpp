...
void BluetoothDevice::updateProperties() {
    ...
    } else if (rawIcon.contains("computer") || rawIcon.contains("laptop") || rawIcon.contains("monitor")) {
        newIcon = "monitor";
        ...
        for (auto it = objects.constBegin(); it != objects.constEnd(); ++it) {
            if (it.value().contains("org.bluez.Device1")) {
                ...
                if (path.startsWith(m_adapterPath)) {
                    ...
                    qDebug() << "[BluetoothManager] Device skipped (path mismatch):" << path << "vs" << m_adapterPath;
                }
            }
        }
...
void BluetoothManager::onDeviceAdded(const QDBusObjectPath &objectPath, const InterfaceMap &interfaces) {
...
void BluetoothManager::onDeviceRemoved(const QDBusObjectPath &objectPath, const QStringList &interfaces) {
    QString path = objectPath.path();
