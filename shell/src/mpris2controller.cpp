...
void MPRIS2Controller::scanForPlayers()
{
    ...
    if (mprisPlayers != m_availablePlayers) {
        ...
        qDebug() << "[MPRIS2Controller] Found" << mprisPlayers.size() << "media players:" << mprisPlayers;
    }
...
void MPRIS2Controller::connectToPlayer(const QString& busName)
{
    ...
    if (!m_playerInterface->isValid()) {
        qWarning() << "[MPRIS2Controller] Failed to connect to" << busName << ":" << m_playerInterface->lastError().message();
...
void MPRIS2Controller::updateCapabilities()
{
    ...
    if (canPlay != m_canPlay) { m_canPlay = canPlay; changed = true; }
    if (canPause != m_canPause) { m_canPause = canPause; changed = true; }
    if (canGoNext != m_canGoNext) { m_canGoNext = canGoNext; changed = true; }
    if (canGoPrevious != m_canGoPrevious) { m_canGoPrevious = canGoPrevious; changed = true; }
    if (canSeek != m_canSeek) { m_canSeek = canSeek; changed = true; }
...
{
    if (!m_playerInterface || !m_canPlay) return;
...
{
    if (!m_playerInterface || !m_canPause) return;
...
{
    if (!m_playerInterface || !m_canSeek) return;
...
QStringList MPRIS2Controller::extractMetadataStringList(const QVariantMap& metadata, const QString& key)
{
    if (metadata.contains(key)) {
