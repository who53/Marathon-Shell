...
bool PortalManager::isPortalAvailable(const QString &portalName) {
    ...
    QString interface;
    if (portalName == "camera")
        interface = PORTAL_CAMERA_INTERFACE;
    else if (portalName == "location")
        interface = PORTAL_LOCATION_INTERFACE;
    else
        return true; // Assume available if generic
...
void PortalManager::requestCameraAccess(const QString &appId) {
    ...
    connect(watcher, &QDBusPendingCallWatcher::finished, this,
            &PortalManager::onCameraRequestFinished);
}
...
class PortalRequest : public QObject {
  ...
  public:
    PortalRequest(const QString &path, const QString &appId, PortalManager *manager)
        ...
        , m_appId(appId)
        , m_manager(manager) {
        QDBusConnection::sessionBus().connect(PORTAL_SERVICE, m_path, PORTAL_REQUEST_INTERFACE,
                                              "Response", this,
...
void PortalManager::requestLocationAccess(const QString &appId) {
    ...
    QDBusMessage message = QDBusMessage::createMethodCall(
        PORTAL_SERVICE, PORTAL_PATH, PORTAL_LOCATION_INTERFACE, "CreateSession");
    ...
    watcher->setProperty("appId", appId);
    connect(watcher, &QDBusPendingCallWatcher::finished, this,
            &PortalManager::onLocationRequestFinished);
}
...
void PortalManager::handlePortalResponse(const QString &path, const QString &appId, uint response,
